<template>
  ...
  <el-menu-item  index="0">正常</el-menu-item>
...
<el-dialog title="添加用户" :visible.sync="addUserShowDialog">
  <el-form :model="form">
    ...
    <el-form-item label="密码" >
     <el-input v-model="form.pow" autocomplete="on"></el-input>
    </el-form-item>
    <el-form-item label="确认密码" >
      <el-input v-model="form.pow2"></el-input>
    </el-form-item>
  </el-form>
...
<el-dialog title="修改用户" :visible.sync="updateUserShowDialog">
  <el-form :model="formUp">
    ...
    <el-form-item label="密码" >
     <el-input v-model="formUp.pow" autocomplete="on"></el-input>
    </el-form-item>
...
export default {
    ...
    methods: {
      ...
      saveUser(){
         let formData=new FormData();
        formData.append("userState",this.userState);
        formData.append("page",this.form.userName);
       // alert(this.currentPage);
      ...
      selectUser(){
        ...
        formData.append("userState",this.userIndex);
        //formData.append("page",this.currentPage);
       // alert(this.currentPage);
        this.$axios.post(this.GLOBAL.serverSrc+'/hostAdmin/select',formData).then(res=> {
              console.log(res);//数据先转换格式
               let data = JSON.parse(res.data.data);
                this.pagetotal = data[0].size;
            var redate =
            typeof data[0].data == "string"
              ? JSON.parse(data[0].data)
              : data[0].data;
          this.tableData = redate;
